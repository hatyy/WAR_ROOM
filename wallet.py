...
cursor = conn.cursor()
req=cursor.execute('SELECT * FROM [WALLET_COMMISSIONNE]')
...
df['sale_date']=df['ladate']

df=df[['WALLET_COM','sale_date','Territory']]


df_obj =pd.read_csv('C:\\Users\\sidyndiaye\\Desktop\\dash_h\\wallet_objectifs.csv',sep=';')

df['Territory'] = df['Territory'].str.lstrip()
df['Territory'] = df['Territory'].str.rstrip()

ll=df[['Territory','sale_date','WALLET_COM']]
...
sd=sd[['Year','Month','NOM','WALLET_COM','objectifs']]
...
lep_df['R_O'] = ((lep_df['WALLET_COM']) / (lep_df['objectifs'].astype(int))).round(decimals=2)
...
df['Previous'] = df.groupby([df['sale_date'].dt.month,df['sale_date'].dt.day])['WALLET_COM'].shift()
##YOY%
fd_ =df[['Territory','sale_date','WALLET_COM']]

fd_['Prev_m'] = fd_.groupby([fd_['sale_date'].dt.month,fd_['sale_date'].dt.day])['WALLET_COM'].shift()
fd_['Year'] = pd.DatetimeIndex(fd_['sale_date']).year

all = fd_.groupby(['Year','Territory'],as_index=False).sum()

all['YOY%'] = (((all['WALLET_COM']-all['Prev_m'])/all['Prev_m'])*100).round(decimals=3)


fd_m=fd_ =df[['Territory','sale_date','WALLET_COM']]

datetime_objects = pd.DatetimeIndex(fd_m['sale_date'])

fd_m['Month'] = datetime_objects.month_name()

fd_m['Prev_m'] = fd_.groupby([fd_['sale_date'].dt.year,fd_['sale_date'].dt.month])['WALLET_COM'].shift()

fd_m['Year'] = pd.DatetimeIndex(fd_m['sale_date']).year

all_m = fd_m.groupby(['Year','Month','Territory'],as_index=False).sum()

all_m['MOM%'] = (((all_m['WALLET_COM'] - all_m['Prev_m'])/all_m['Prev_m'])*100).round(decimals=2)
...
all_m['R_O'] = ((all_m['WALLET_COM']) / (all_m['objectifs'].astype(int))).round(decimals=2)
...
for i in periode_m: 
    if(i==m):
        break
val_2018 += df[(df['Year'] == int(t)-1)&(df['Month'] == i)]['WALLET_COM'].sum()
val_2019 += df[(df['Year'] == int(t))&(df['Month'] == i)]['WALLET_COM'].sum()
...
fd_s=fd_ =df[['Territory','sale_date','WALLET_COM']]
...
fd_s=fd_s[['Territory','Year','Month','Week','WALLET_COM']]
fd_s=fd_s.groupby(['Territory','Year','Month','Week'],as_index=False).sum()
fd_s['Prev_m'] = fd_s.groupby([fd_s['Territory']])['WALLET_COM'].shift(1)
fd_s=fd_s.fillna(0)
fd_s['base']=fd_s['WALLET_COM']-fd_s['Prev_m']
...
_navbar = dbc.Navbar(  
    ...
    children=[
        ...
        dbc.DropdownMenu(
            ...
            children=[
                ...
                dbc.DropdownMenuItem(divider=True),
                dbc.DropdownMenuItem("WALLET",href='#'),
    ...
    [
        dbc.Col(
            dbc.Button("ALL TERRYTORY WALLET_COM ", color="primary", className="ml-2",block=True),
            width="500px",
        ),
    ],
    no_gutters=True,
...
html.Div([
        dcc.Graph(id='map-display-wallet'),    
...
style_data_conditional=[
    ...
    dcc.Graph(id='funnel-graph-wallet',style={"height" : "25%", "width" : "40%"}),
    dcc.Graph(id='funnel-wallet',style={"height" : "25%", "width" : "40%"})  
        ,
    ])
])
@app.callback(
    dash.dependencies.Output('map-display-wallet', 'figure'),
...
@app.callback(
    dash.dependencies.Output('funnel-graph-wallet', 'figure'),
...
def update_graph(Year,month):
    ...
    pv = pd.pivot_table(
        df_plot,
        index=['Territory'],
        values=['WALLET_COM','Previous'],
        aggfunc=sum,
        fill_value=0)

    MTD = go.Bar(x=pv.index, y=pv['WALLET_COM'],name='WALLET_COM',marker_color='rgb(55, 83, 109)')
    ...
    return {
        ...
        'layout':
        go.Layout(
            title=' WALLET_COM MOM% {}'.format(month),
            barmode='group')
  }
        
@app.callback(
     dash.dependencies.Output('funnel-wallet', 'figure'),
...
def updates_graphs(Year):
    ...
    return {
        ...
        'layout':
        go.Layout(
            title=' WALLET_COM YOY% {}'.format(Year),
            barmode='group')
  }   
...
@app.callback(
    dash.dependencies.Output('datatable-paging-wallet', 'data'),
