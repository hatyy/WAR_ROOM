# -*- coding: utf-8 -*-
"""
Created on Thu Feb  6 10:56:11 2020
...
cursor = conn.cursor()
req=cursor.execute('SELECT * FROM SWAPP')
...
df_obj =pd.read_csv('C:\\Users\\sidyndiaye\\Desktop\\dash_h\\objectifs_swapp.csv',sep=';')

df['Territory'] = df['Territory'].str.lstrip()
df['Territory'] = df['Territory'].str.rstrip()

ll=df[['Territory','sale_date','nbr']]
...
sd=sd[['Year','Month','NOM','nbr','objectifs']]
...
lep_df['R_O'] = ((lep_df['nbr']) / (lep_df['objectifs'].astype(int))).round(decimals=2)
...
df['Previous'] = df.groupby([df['sale_date'].dt.month,df['sale_date'].dt.day])['nbr'].shift()
##YOY%
fd_ =df[['Territory','sale_date','nbr']]

fd_['Prev_m'] = fd_.groupby([fd_['sale_date'].dt.month,fd_['sale_date'].dt.day])['nbr'].shift()
fd_['Year'] = pd.DatetimeIndex(fd_['sale_date']).year

all = fd_.groupby(['Year','Territory'],as_index=False).sum()

all['YOY%'] = (((all['nbr']-all['Prev_m'])/all['Prev_m'])*100).round(decimals=3)


fd_m=fd_ =df[['Territory','sale_date','nbr']]

datetime_objects = pd.DatetimeIndex(fd_m['sale_date'])

fd_m['Month'] = datetime_objects.month_name()

fd_m['Prev_m'] = fd_.groupby([fd_['sale_date'].dt.year,fd_['sale_date'].dt.month])['nbr'].shift()

fd_m['Year'] = pd.DatetimeIndex(fd_m['sale_date']).year

all_m = fd_m.groupby(['Year','Month','Territory'],as_index=False).sum()

all_m['MOM%'] = (((all_m['nbr'] - all_m['Prev_m'])/all_m['Prev_m'])*100).round(decimals=2)
...
all_m['R_O'] = ((all_m['nbr']) / (all_m['objectifs'].astype(int))).round(decimals=2)
...
for i in periode_m:
    if(i==m):
        break
    val_2018 +=   df[(df['Year'] == int(t)-1)&(df['Month'] == i)]['nbr'].sum()
    val_2019 += df[(df['Year'] == int(t))&(df['Month'] == i)]['nbr'].sum()
...
fd_s=fd_ =df[['Territory','sale_date','nbr']]
...
fd_s=fd_s[['Territory','Year','Month','Week','nbr']]
fd_s=fd_s.groupby(['Territory','Year','Month','Week'],as_index=False).sum()
fd_s['Prev_m'] = fd_s.groupby([fd_s['Territory']])['nbr'].shift(1)
fd_s=fd_s.fillna(0)
fd_s['base']=fd_s['nbr']-fd_s['Prev_m']
...
_navbar = dbc.Navbar(  
    ...
    children=[
        ...
        dbc.DropdownMenu(
            ...
            children=[
                ...
                dbc.DropdownMenuItem("SWAP",href='#'),
    ...
    [
        dbc.Col(
            dbc.Button("ALL TERRYTORY SWAP", color="primary", className="ml-2",block=True),
            width="500px",
        ),
    ],
    no_gutters=True,
...
style_data_conditional=[
    ...
    dcc.Graph(id='funnel_swap-graph2',style={"height" : "25%", "width" : "40%"}),
    dcc.Graph(id='funnel_swap',style={"height" : "25%", "width" : "40%"})  
        ,
    ])
])
@app.callback(
    dash.dependencies.Output('map-display_swap', 'figure'),
...
@app.callback(
    dash.dependencies.Output('funnel_swap-graph2', 'figure'),
...
def update_graph(Year,month):
    ...
    MTD = go.Bar(x=pv.index, y=pv['nbr'],name='SWAP',marker_color='rgb(55, 83, 109)')
    ...
    return {
        ...
        'layout':
        go.Layout(
            title=' SWAP MOM% {}'.format(month),
            barmode='group')
  }
        
@app.callback(
     dash.dependencies.Output('funnel_swap', 'figure'),
...
def updates_graphs(Year):
    ...
    MTD = go.Bar(x=pv.index, y=pv['nbr'],name='SWAPP', marker_color='rgb(55, 83, 109)')
    MTD_1 = go.Bar(x=pv.index, y=pv['Previous'],name='PREVIOUS', marker_color='rgb(26, 118, 255)')
...
@app.callback(
    dash.dependencies.Output('datatable-paging2', 'data'),
