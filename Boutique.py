...
req=cursor.execute("SELECT * FROM ALL_GROSS WHERE ROLE like '%Boutique%' ")
...
table_poa['BOUTIQUE'] = table_poa['PhoneNumber']

del table_poa['PhoneNumber']
table_poa=table_poa[['Territory','Year','Month','BOUTIQUE','MOM%']]
...
############################ 
, 
    dcc.Graph(id='funnel-graph-Boutique',style={"height" : "25%", "width" : "40%"}),
    dcc.Graph(id='funnel-Boutique',style={"height" : "25%", "width" : "40%"}),
    html.Div(   )
        ,
    ])
])
...
@app.callback(
    dash.dependencies.Output('funnel-graph-Boutique', 'figure'),
...
@app.callback(
     dash.dependencies.Output('funnel-Boutique', 'figure'),
...
@app.callback(
    dash.dependencies.Output('datatable-paging-Boutique', 'data'),
...
@app.callback(
    dash.dependencies.Output('datatable-ACTIF-Boutique', 'data'),
    [dash.dependencies.Input('Territory', 'value'),dash.dependencies.Input('month', 'value')])
