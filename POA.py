...
req=cursor.execute("SELECT * FROM ALL_GROSS WHERE ROLE like 'POA' ")
...
table_poa['POA'] = table_poa['PhoneNumber']

del table_poa['PhoneNumber']
table_poa=table_poa[['Territory','Year','Month','POA','MOM%']]
    ...
    children=[
        ...
        dbc.DropdownMenu(
            ...
            children=[
                ...
                dbc.DropdownMenuItem("BOUTIQUE",href='#'),
                dbc.DropdownMenuItem(divider=True),
                dbc.DropdownMenuItem("POA",href='#'),
...
############################ 
, 
    dcc.Graph(id='funnel-graph-POA',style={"height" : "25%", "width" : "40%"}),
    dcc.Graph(id='funnel-POA',style={"height" : "25%", "width" : "40%"}),
    html.Div(   )
        ,
    ])
])
...
@app.callback(
    dash.dependencies.Output('funnel-graph-POA', 'figure'),
...
@app.callback(
     dash.dependencies.Output('funnel-POA', 'figure'),
...
@app.callback(
    dash.dependencies.Output('datatable-paging-POA', 'data'),
...
@app.callback(
    dash.dependencies.Output('datatable-ACTIF-POA', 'data'),
    [dash.dependencies.Input('Territory', 'value'),dash.dependencies.Input('month', 'value')])
