...
df= df.groupby(['Territory','sale_date','Year','Month','ZONE_COM'],as_index=False).sum()

df['Previous'] = df.groupby([df['Year'],df['Month']])['GROSS'].shift()
##YOY%
...
fd_m=fd_ =df[['Territory','ZONE_COM','sale_date','GROSS']]
...
all_m = fd_m.groupby(['Year','Month','Territory','ZONE_COM'],as_index=False).sum()
...
for i in periode_m:
    if(i==m):
        break
    val_2018 +=   df[(df['Year'] == int(t)-1)&(df['Month'] == i)]['GROSS'].sum()
    val_2019 += df[(df['Year'] == int(t))&(df['Month'] == i)]['GROSS'].sum()
...
fd_s=fd_s.groupby(['Territory','Week','Year','Month'],as_index=False).sum()
...
trace_1 = go.Scatter(x = fd_s.Week, y = fd_s['GROSS'],
                    name = 'GROSS',
                    line = dict(width = 2,
                                )) 
...
tracex = go.Waterfall(
    ...
    y = fd_s['GROSS'], connector = {"mode":"between", "line":{"width":4, "color":"rgb(0, 0, 0)", "dash":"solid"}},
...
style_data_conditional=[
    ...
    dcc.Graph(id='funnels-graphs',style={"height" : "25%", "width" : "40%"}),
    dcc.Graph(id='funnels',style={"height" : "25%", "width" : "40%"}),
    dcc.Graph(id = 'plot', figure = fig,style={"height" : "50%", "width" : "60%","position":"absolute","left":"42%","top":"95%"}),
    ])
])
@app.callback(
    dash.dependencies.Output('display', 'figure'),
...
@app.callback(
    [dash.dependencies.Output('datatable-paging1', 'data'),dash.dependencies.Output('funnels-graphs', 'figure'),dash.dependencies.Output('funnels', 'figure'),
...
def updates_table(Year,month,zone_com):
    ...
    MTD_m = go.Bar(x=pv.index, y=pv['GROSS'],name='GROSS',marker_color='rgb(55, 83, 109)')
    ...
    trace_1 = go.Waterfall(x = st2.Week,  measure = ["relative", "relative", "relative", "relative", "total", "relative",
                                              "relative", "relative", "relative", "total", "relative", "relative", "total", "relative", "total"],
                           y = st2['GROSS'],
    ...
    return all_plot.to_dict('records'),{
        ...
        'layout':
        go.Layout(
            title=' GROSS YOY% {}'.format(Year),
            barmode='group')
  },{
        'data': [MTD_m,MTD_1_m],
        'layout':
        go.Layout(
            title=' GROSS MOM% {}'.format(Year),
            barmode='group')
  },fig 
