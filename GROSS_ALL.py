...
periode_m= ['Janvier', 'Février', 'Mars','Avril', 'Mai', 'Juin','Juillet', 'Aout', 'Septembre','Octobre','Novembre','Decembre']
...
col = 'De Jan à : '  +""+ maxs.strftime('%B %d, %Y, %r')
col2 ='De Jan à : '+ ""+date(maxs.year - 1, maxs.month, maxs.day).strftime('%B %d, %Y, %r')
...
_navbar = dbc.Navbar(  
    ...
    children=[
        ...
        dbc.DropdownMenu(
            ...
            label="S&D Telco",
            children=[
                dbc.DropdownMenuItem('GROSS ALL', href='/GROSS_ALL_TERRITORY'),
...
labels = df['Territory']
values = df['GROSS']
colors = ['#FEBFB3', '#E1396C', '#96D38C', '#D0F9B1','#D0F6B1','#D6F9B1','#D7F9B2','#D0F9C1']
...
style_data_conditional=[
    ...
    dcc.Graph(id='funnel-graph',style={"height" : "25%", "width" : "40%"}),
    dcc.Graph(id='funnel',style={"height" : "25%", "width" : "40%"}),
    dcc.Graph(id = 'circle', figure = fig,style={"height" : "50%", "width" : "60%","position":"absolute","left":"42%","top":"95%"})
        ,
    ])
])
@app.callback(
    dash.dependencies.Output('map-display', 'figure'),[dash.dependencies.Input('month', 'value')])
...
def updates_table(Year,month):
    ...
    if Year == "All Year":
        ...
        all_plot =all_m[(all_m.Year == int(t))&(all_m.Month == m)].to_dict('records').copy()
...
@app.callback(
    dash.dependencies.Output('circle', 'figure'),
    
    [dash.dependencies.Input('Territory', 'value'),dash.dependencies.Input('month', 'value')])

def update_pie_graphe(Year,month) :
    
    if Year == Year :
        pie_graph = df[df['Year'] == Year]
        labels = pie_graph['Territory']
        values = pie_graph['GROSS']
        colors = ['#FEBFB3', '#E1396C', '#96D38C', '#D0F9B1','#D0F6B1','#D6F9B1','#D7F9B2','#D0F9C1']     
        ...
        pie_graph = df[(df['Year'] == Year)&(df['Month'] == month)]
        labels = pie_graph['Territory']
        values = pie_graph['GROSS']
        colors = ['#FEBFB3', '#E1396C', '#96D38C', '#D0F9B1','#D0F6B1','#D6F9B1','#D7F9B2','#D0F9C1']     
        traces  = go.Pie(labels=labels, values = values,
