...
cursor = conn.cursor()
req=cursor.execute('SELECT * FROM [EMoneyInjecte]')
...
df1['FROMAH_MSISDN_DD']=df1['numero bis']
df1['Territory']=df1['TERRITOIRE']

df1=df1[['Territory','FROMAH_MSISDN_DD']]

df['FROMAH_MSISDN_DD'] = df['FROMAH_MSISDN_DD'].str.lstrip()
df['FROMAH_MSISDN_DD'] = df['FROMAH_MSISDN_DD'].str.rstrip()

##convert int type to string 

df1.FROMAH_MSISDN_DD = df1.FROMAH_MSISDN_DD.astype('str')

df1['FROMAH_MSISDN_DD'] = df1['FROMAH_MSISDN_DD'].str.lstrip()
df1['FROMAH_MSISDN_DD'] = df1['FROMAH_MSISDN_DD'].str.rstrip()

df = pd.merge(df,df1,on='FROMAH_MSISDN_DD')
...
##convert int to date 
df['sale_date'] = df['annee'].apply(lambda x: pd.to_datetime(str(x), format='%Y%m%d'))

df=df[['Territory','sale_date','EMoneyInjecter']]
#object to int conversion
df['EMoneyInjecter']=pd.to_numeric(df['EMoneyInjecter'])
...
ll=df[['Territory','sale_date','EMoneyInjecter']]
...
sd=sd[['Year','Month','NOM','EMoneyInjecter','objectifs']]
...
lep_df['R_O'] = ((lep_df['EMoneyInjecter']) / (lep_df['objectifs'].astype(int))).round(decimals=2)
...
df['Previous'] = df.groupby([df['sale_date'].dt.month,df['sale_date'].dt.day])['EMoneyInjecter'].shift()
##YOY%
fd_ =df[['Territory','sale_date','EMoneyInjecter']]

fd_['Prev_m'] = fd_.groupby([fd_['sale_date'].dt.month,fd_['sale_date'].dt.day])['EMoneyInjecter'].shift()
fd_['Year'] = pd.DatetimeIndex(fd_['sale_date']).year

all = fd_.groupby(['Year','Territory'],as_index=False).sum()

all['YOY%'] = (((all['EMoneyInjecter']-all['Prev_m'])/all['Prev_m'])*100).round(decimals=3)


fd_m=fd_ =df[['Territory','sale_date','EMoneyInjecter']]

datetime_objects = pd.DatetimeIndex(fd_m['sale_date'])

fd_m['Month'] = datetime_objects.month_name()

fd_m['Prev_m'] = fd_.groupby([fd_['sale_date'].dt.year,fd_['sale_date'].dt.month])['EMoneyInjecter'].shift()

fd_m['Year'] = pd.DatetimeIndex(fd_m['sale_date']).year

all_m = fd_m.groupby(['Year','Month','Territory'],as_index=False).sum()

all_m['MOM%'] = (((all_m['EMoneyInjecter'] - all_m['Prev_m'])/all_m['Prev_m'])*100).round(decimals=2)
...
all_m['R_O'] = ((all_m['EMoneyInjecter']) / (all_m['objectifs'].astype(int))).round(decimals=2)
...
for i in periode_m:
    if(i==m):
        break
    val_2018 +=  df[(df['Year'] == int(t)-1)&(df['Month'] == i)]['EMoneyInjecter'].sum()
    val_2019 +=df[(df['Year'] == int(t))&(df['Month'] == i)]['EMoneyInjecter'].sum()
...
col2 ='De Jan à : '+ ""+date(max_date.year - 1, mydate.month, mydate.day).strftime('%B %d, %Y, %r')
...
fd_s=fd_ =df[['Territory','sale_date','EMoneyInjecter']]
...
fd_s=fd_s[['Territory','Year','Month','Week','EMoneyInjecter']]
fd_s=fd_s.groupby(['Territory','Year','Month','Week'],as_index=False).sum()
fd_s['Prev_m'] = fd_s.groupby([fd_s['Territory']])['EMoneyInjecter'].shift(1)
fd_s=fd_s.fillna(0)
fd_s['base']=fd_s['EMoneyInjecter']-fd_s['Prev_m']
...
_navbar = dbc.Navbar(  
    ...
    children=[
        ...
        dbc.DropdownMenu(
            ...
            children=[
                ...
                dbc.DropdownMenuItem("EMONEY INJECTE",href='#'),
                dbc.DropdownMenuItem(divider=True),
    ...
    [
        dbc.Col(
            dbc.Button("ALL TERRYTORY EMoneyInjecter", color="primary", className="ml-2",block=True),
            width="500px",
        ),
    ],
    no_gutters=True,
...
style_data_conditional=[
    ...
    dcc.Graph(id='funnel-graph_INJ',style={"height" : "25%", "width" : "40%"}),
    dcc.Graph(id='funnel_INJ',style={"height" : "25%", "width" : "40%"})  
        ,
    ])
])
@app.callback(
    dash.dependencies.Output('map-display_INJ', 'figure'),
...
@app.callback(
    dash.dependencies.Output('funnel-graph_INJ', 'figure'),
...
def update_graph(Year,month):
    ...
    pv = pd.pivot_table(
        df_plot,
        index=['Territory'],
        values=['EMoneyInjecter','Previous'],
        aggfunc=sum,
        fill_value=0)

    MTD = go.Bar(x=pv.index, y=pv['EMoneyInjecter'],name='GROSS',marker_color='rgb(55, 83, 109)')
    ...
    return {
        ...
        'layout':
        go.Layout(
            title=' EMoneyInjecter MOM% {}'.format(month),
            barmode='group')
  }
        
@app.callback(
     dash.dependencies.Output('funnel_INJ', 'figure'),
...
def updates_graphs(Year):
    ...
    MTD = go.Bar(x=pv.index, y=pv['EMoneyInjecter'],name='EMoneyInjecter', marker_color='rgb(55, 83, 109)')
    MTD_1 = go.Bar(x=pv.index, y=pv['Previous'],name='PREVIOUS', marker_color='rgb(26, 118, 255)')
    ...
    return {
        ...
        'layout':
        go.Layout(
            title=' EMoneyInjecter YOY% {}'.format(Year),
            barmode='group')
  }   
...
@app.callback(
    dash.dependencies.Output('datatable-paging_INJ', 'data'),
